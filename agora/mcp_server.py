"""The surface an agent CLI sees: `python -m agora.mcp_server --agent <name>`.
...
**There is no `agora_decide` tool.** Not a disabled one, not one that checks a
...
decide through `agora approve` / the web UI, on a different surface entirely.
...
# tool parameter.
AGENT: str = os.environ.get("AGORA_AGENT", "unknown")
...
@mcp.tool()
def agora_read(topic: str, after: int = 0) -> str:
...
@mcp.tool()
def agora_say(topic: str, body: str) -> str:
...
@mcp.tool()
def agora_propose(topic: str, title: str, body: str) -> str:
...
@mcp.tool()
def agora_vote(proposal_id: int, stance: str, rationale: str = "") -> str:
...
@mcp.tool()
def agora_ask(topic: str, agent: str, question: str) -> str:
    ...
    Writing `@name` inside `agora_say` does the same thing.
...
@mcp.tool()
def agora_pass(topic: str, why: str = "nothing to add") -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--agent", default=os.environ.get("AGORA_AGENT"),
                    help="seat name this CLI posts as; binds identity for the session")
    ap.add_argument("--db", default=os.environ.get("AGORA_DB"), help="path to board.db")
    args = ap.parse_args(argv)

    if not args.agent:
        ap.error("--agent is required (or set AGORA_AGENT)")
    ...
    try:
        ...
        print(f"agora: unknown seat {AGENT!r}; run `agora agents add {AGENT} <kind>` first",
              file=sys.stderr)
        return 2
