"""Register the Agora MCP server with the CLIs that cannot take it per-run.
...
(`agora-<seat>`), not once globally, because the agent identity is bound in the
...
def server_argv(agent: str, db: Path | str) -> list[str]:
    ...
    return [sys.executable.replace("\\", "/"), "-X", "utf8", "-m", "agora.mcp_server",
...
def install_seat(store: Store, agent: str, *, dry_run: bool = False) -> tuple[bool, str]:
    ...
    return True, f"registered as agora-{agent}"
